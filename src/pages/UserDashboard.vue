<template>
  <div class="min-h-screen" style="background: #f5f7fa">
    <div class="max-w-[1600px] mx-auto px-4 md:px-8 py-4 md:py-6">
      <!-- Header -->
      <div
        class="rounded-lg p-5 md:p-6 mb-5 md:mb-6 text-white shadow-md"
        style="background: #002147; border-bottom: 4px solid #fbbf24"
      >
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
          <div class="flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full md:w-auto">
            ...
            <div class="text-center md:text-left md:ml-2">
              ...
              <p class="text-xs md:text-sm font-semibold" style="color: #fbbf24">
                PNP Police Station 1
              </p>
              <p
                v-if="selectedActivityName"
          ...
          <button
            @click="returnToLogin"
            class="text-white font-bold px-5 md:px-6 py-2.5 md:py-3 rounded-lg flex items-center gap-2 hover:opacity-90 transition w-full md:w-auto justify-center"
        ...
        <div class="lg:col-span-3 order-1 space-y-4">
          ...
          <div class="bg-white rounded-lg p-5 shadow-md">
            <div class="flex items-center gap-3 mb-4">
              ...
              <h2 class="text-base md:text-lg font-bold" style="color: #002147">Instructions</h2>
            </div>
            <div class="space-y-3">
              ...
              <div class="flex gap-3">
                ...
                <p class="text-xs md:text-sm text-gray-700">Review your information carefully</p>
              </div>
              <div class="flex gap-3">
                <div
                  class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold"
          ...
          <div
            class="hidden lg:block bg-amber-50 rounded-lg p-4 shadow-md border-l-4 border-amber-400"
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                <h3 class="font-bold text-sm md:text-base mb-2" style="color: #92400e">
                  Important Notice
                </h3>
                <p class="text-xs md:text-sm" style="color: #92400e">
          ...
          <div class="hidden lg:block bg-white rounded-lg p-4 shadow-md">
            <h3 class="font-bold text-sm md:text-base mb-3" style="color: #002147">
              System Information
            </h3>
            <div class="space-y-2">
            ...
            <!-- Activity Submission Limit Info -->
            <div v-if="selectedActivityId" class="mt-4 pt-4 border-t" style="border-color: #e5e7eb">
              <p class="text-xs font-bold mb-2" style="color: #002147">Submission Status</p>
              <div class="flex items-center gap-2">
                <div class="flex-1 bg-gray-200 rounded-full h-2" style="background: #e5e7eb">
                  <div
                    class="h-2 rounded-full transition-all"
                    :style="{
                      width:
                        Math.min((currentUserSubmissionCount / maxSubmissions) * 100, 100) + '%',
                      background:
                        currentUserSubmissionCount >= maxSubmissions ? '#dc2626' : '#10b981',
                    }"
                  ></div>
                </div>
              </div>
              <p class="text-xs mt-2 font-semibold" style="color: #002147">
                {{ currentUserSubmissionCount }} / {{ maxSubmissions }} Submitted
              </p>
              <p
                v-if="currentUserSubmissionCount < maxSubmissions"
                class="text-xs text-green-600 font-bold mt-1"
              >
                ✓ {{ maxSubmissions - currentUserSubmissionCount }} submission{{
                  maxSubmissions - currentUserSubmissionCount !== 1 ? 's' : ''
                }}
                remaining
              </p>
              <p v-else class="text-xs font-bold mt-1" style="color: #dc2626">
        ...
        <div class="lg:col-span-9 order-2">
          <div class="bg-white rounded-lg p-6 space-y-5 shadow-md">
            <div
              ...
              style="background: #eff6ff; border-left: 4px solid #002147"
            >
              <div class="rounded-lg p-2" style="background: #002147">
                <svg
              ...
              <div>
                <p class="text-sm md:text-base font-bold" style="color: #002147">
            ...
            <div
              v-if="fetchError"
              class="border-l-4 p-4 rounded-lg text-xs md:text-sm font-bold"
            ...
            <div>
              <label
                class="text-sm md:text-base font-bold mb-3 flex items-center gap-2"
                style="color: #002147"
              >
                <svg
              ...
              <div class="relative">
                <input
                  ...
                  class="w-full px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 transition font-medium"
                  style="border-color: #d1d5db; color: #002147"
                  autocomplete="off"
                />
                ...
                <div
                  ...
                  class="absolute z-10 w-full mt-2 bg-white border-2 rounded-lg max-h-64 overflow-y-auto shadow-lg"
                  style="border-color: #d1d5db"
                >
                  <div
                    v-if="filteredUsers.length === 0 && !shouldShowAddNewOfficer"
                  ...
                  <div
                    ...
                    :class="{ 'font-bold bg-blue-100': selectedUserId === user.id }"
                    :style="selectedUserId === user.id ? 'color: #002147;' : ''"
                  >
                    {{ user.rank_full_name }}
                  </div>
                  <!-- Add New Officer Option -->
                  <div
                    v-if="shouldShowAddNewOfficer"
                    @mousedown.prevent="startAddNewOfficer"
                    class="px-3 md:px-4 py-2.5 md:py-3 hover:bg-green-50 cursor-pointer text-xs md:text-sm border-t transition font-bold flex items-center gap-2"
                    style="color: #059669"
                  >
                    ...
                    Add "{{ searchQuery }}" as new officer
                  </div>
                </div>
              ...
              <!-- Add New Officer Form Modal -->
              <div
                v-if="showAddOfficerForm"
                class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
              >
                <div class="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
                  <h3 class="text-lg font-bold mb-4" style="color: #002147">Add New Officer</h3>

                  <!-- Error Message in Modal -->
                  <div
                    v-if="fetchError"
                    class="mb-4 p-3 text-xs font-bold rounded-lg"
                    style="background: #fee2e2; color: #dc2626; border-left: 4px solid #dc2626"
                  >
                    {{ fetchError }}
                  </div>
                  ...
                  <div class="mb-4">
                    <label class="block text-xs font-bold mb-2" style="color: #002147">
                      Officer Name <span class="text-red-600">*</span>
                    </label>
                    <input
                      v-model="newOfficerName"
                      type="text"
                      placeholder="Enter officer's full name"
                      class="w-full px-3 py-2 text-sm border-2 rounded-lg focus:outline-none focus:ring-2"
                      style="border-color: #d1d5db; color: #002147"
                    />
                  </div>
                  ...
                  <div class="flex gap-3">
                    <button
                      @click="cancelAddNewOfficer"
                      class="flex-1 px-4 py-2 text-xs font-bold uppercase rounded-lg border-2 transition"
                    ...
                    <button
                      @click="addNewOfficer"
                      class="flex-1 px-4 py-2 text-xs font-bold uppercase rounded-lg text-white transition"
                      style="background: #059669"
                      :disabled="isAddingOfficer || !newOfficerName.trim()"
                    >
                      <span v-if="!isAddingOfficer">Add Officer</span>
              ...
              <div
                class="mt-3 px-4 py-3 rounded-lg text-xs md:text-sm"
              ...
              >
                <div class="flex items-center justify-between">
                  ...
                  <button
                    ...
                    class="text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:opacity-90 transition"
                    style="background: #dc2626"
                  >
                    Clear
                  </button>
              ...
              <p v-if="!isLoadingUsers && allUsers.length > 0" class="text-xs text-gray-500 mt-2">
            ...
            <div>
              ...
              >
                ...
                >
                  <div
                    class="rounded-lg p-4 mx-auto w-16 h-16 flex items-center justify-center mb-3"
                    style="background: #002147"
                  >
                    <svg
                ...
                <div v-else class="space-y-4">
                  ...
                  <div class="flex gap-3 overflow-x-auto pb-3">
                    <div
                      ...
                      :key="index"
                      class="relative shrink-0 bg-white rounded border hover:border-gray-400 transition"
                    ...
                    >
                      <div class="relative">
                        <img
                          ...
                          :src="preview.url"
                          alt="Preview"
                          class="w-28 h-28 md:w-32 md:h-32 object-cover rounded cursor-zoom-in"
                        />
                        <button
                          @click.stop="removeImage(index)"
                          class="absolute -top-2 -right-2 text-white rounded-full p-1.5 transition hover:opacity-90"
                          style="background: #dc2626"
                        >
                          <svg
                            class="w-4 h-4"
                        ...
                        <div
                          class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-75 text-white text-xs p-2 rounded-b"
                        >
                  ...
                  <button
                    @click="$refs.fileInput.click()"
                    class="w-full text-white font-bold py-3 rounded-lg text-sm transition hover:opacity-90 flex items-center justify-center gap-2"
            ...
            <div class="pt-4">
              <button
                ...
                class="w-full py-4 text-white font-bold text-base rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition hover:opacity-90"
                style="background: #002147"
              ...
              <div class="mt-4 p-3 rounded-lg text-center" style="background: #fef3c7">
                <p class="text-xs md:text-sm font-bold" style="color: #92400e">
                  <span style="color: #dc2626">*</span> Required fields must be filled out before
                  submitting
                </p>
              </div>
            </div>
        ...
        <div class="lg:hidden lg:col-span-3 order-3 grid grid-cols-1 gap-4">
          ...
          <div class="bg-white rounded-lg p-4 shadow-md">
            ...
            <div class="space-y-2">
              ...
              <!-- Activity Submission Limit Info for Mobile -->
              <div
                v-if="selectedActivityId"
              ...
              >
                <p class="text-xs font-bold mb-2" style="color: #002147">Submission Status</p>
                <div class="flex items-center gap-2">
                  <div class="flex-1 bg-gray-200 rounded-full h-1.5" style="background: #e5e7eb">
                    <div
      ...
      <div
        v-if="showSuccess"
        class="fixed top-4 left-4 right-4 md:top-8 md:left-8 md:right-auto px-5 md:px-6 py-3 md:py-4 rounded-lg z-50 shadow-xl"
      ...
      >
        <button
          @click="closeZoom"
          class="absolute top-4 right-4 md:top-6 md:right-6 text-white rounded p-2 md:p-3 transition hover:opacity-90 z-10"
          style="background: #dc2626"
        >
...
const newOfficerName = ref('')
const newOfficerRank = ref('')
const newOfficerBadge = ref('')
const isAddingOfficer = ref(false)
const showAddOfficerForm = ref(false)
...
const filteredUsers = computed(() => {
  ...
  return allUsers.value.filter((user) => user.rank_full_name.toLowerCase().includes(query))
})

// Check if we should show "Add New Officer" option
const shouldShowAddNewOfficer = computed(() => {
  return searchQuery.value && searchQuery.value.trim() !== '' && filteredUsers.value.length === 0
})
...
const selectUser = (user) => {
  ...
  // Check submission count when user is selected
  checkUserSubmissionCount()
}

const startAddNewOfficer = () => {
  newOfficerName.value = searchQuery.value.trim()
  newOfficerRank.value = ''
  newOfficerBadge.value = ''
  showAddOfficerForm.value = true
}

const addNewOfficer = async () => {
  if (!newOfficerName.value.trim()) {
    fetchError.value = 'Officer name is required'
    return
  }

  isAddingOfficer.value = true
  fetchError.value = ''
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([
        {
          rank_full_name: newOfficerName.value.trim(),
          badge_number: newOfficerBadge.value.trim() || null,
        },
      ])
      .select()

    if (error) {
      throw new Error(`Failed to add officer: ${error.message}`)
    }

    if (data && data.length > 0) {
      const newOfficer = data[0]
      // Add to the users list
      allUsers.value.push(newOfficer)
      // Sort the list
      allUsers.value.sort((a, b) => a.rank_full_name.localeCompare(b.rank_full_name))
      // Auto-select the new officer
      selectUser(newOfficer)
      // Reset form
      showAddOfficerForm.value = false
    ...
    fetchError.value = err.message || 'Failed to add officer'
  } finally {
    isAddingOfficer.value = false
  }
}

const cancelAddNewOfficer = () => {
  showAddOfficerForm.value = false
  newOfficerName.value = ''
  newOfficerRank.value = ''
  newOfficerBadge.value = ''
...
const clearSelection = () => {
  ...
  searchQuery.value = ''
  currentUserSubmissionCount.value = 0
}
...
const submitForm = async () => {
  ...
  // Check if user has exceeded submission limit for this activity
  if (selectedActivityId.value && currentUserSubmissionCount.value >= maxSubmissions.value) {
    fetchError.value = `You have already submitted ${currentUserSubmissionCount.value} time(s) for this activity. Maximum allowed: ${maxSubmissions.value}`
...
// Load activity info if activity_id is provided
const loadActivityInfo = async () => {
  ...
  try {
    ...
    if (error) {
      console.warn('Activity not found:', error)
      return
    }
...
const checkUserSubmissionCount = async () => {
  if (!selectedActivityId.value || !selectedUserId.value) {
    currentUserSubmissionCount.value = 0
    return
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq('activity_id', selectedActivityId.value)

    if (error) {
      console.warn('Error checking submission count:', error)
      return
    }

    currentUserSubmissionCount.value = data?.length || 0
    console.log('User submission count:', currentUserSubmissionCount.value)
  } catch (err) {
